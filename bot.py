...
def main():
    ...
    if WEBHOOK_URL:
        ...
        class AdminStatsHandler(tornado.web.RequestHandler):
            ...
            def options(self): self.set_status(200)
            def post(self):
                h = AdminAPIHandler.__new__(AdminAPIHandler)
                import io, json as _j
                body = _j.loads(self.request.body or b"{}")
                user = _validate_init_data(body.get("initData",""))
                if not user or (user.get("username") or "").lower() != ADMIN_USERNAME.lower():
                    self.set_status(401); self.write({"ok":False,"error":"unauthorized"}); return
                self.set_header("Content-Type","application/json")
                self.write(h._get_stats())
        ...
        class AdminUsersHandler(tornado.web.RequestHandler):
            ...
            def post(self):
                ...
                if not user or (user.get("username") or "").lower() != ADMIN_USERNAME.lower():
                    self.set_status(401); self.write({"ok":False,"error":"unauthorized"}); return
                self.set_header("Content-Type","application/json")
                self.write(_j.dumps(h._get_users()))

        class AdminTopHandler(tornado.web.RequestHandler):
            def set_default_headers(self):
                self.set_header("Access-Control-Allow-Origin", "*")
                self.set_header("Access-Control-Allow-Methods", "POST, OPTIONS")
                self.set_header("Access-Control-Allow-Headers", "Content-Type")
            ...
            def post(self):
                ...
                if not user or (user.get("username") or "").lower() != ADMIN_USERNAME.lower():
                    self.set_status(401); self.write({"ok":False,"error":"unauthorized"}); return
                self.set_header("Content-Type","application/json")
                self.write(_j.dumps(h._get_top()))
        ...
        app.run_webhook(
            ...
            custom_updates=[
                ...
                (r"/admin-api/top",   AdminTopHandler),
            ],
        )
    else:
