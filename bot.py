...
import hmac, hashlib, json, threading
...
def init_db():
    with get_conn() as conn:
        with conn.cursor() as cur:
            ...
            """)
            cur.execute("""
                CREATE TABLE IF NOT EXISTS user_settings_db (
                    user_id BIGINT PRIMARY KEY,
                    country TEXT,
                    regions TEXT,
            ...
            """)
            cur.execute("""
                ALTER TABLE user_settings_db ADD COLUMN IF NOT EXISTS frame TEXT DEFAULT 'none'
            """)
            cur.execute("""
...
# ── Сохранение/загрузка настроек бота в БД ────────────────────────────────────
...
def load_bot_settings():
    """Загружает все глобальные настройки из БД при старте."""
    global global_country, COOLDOWN_HOURS, MAINTENANCE_MODE, X2_ACTIVE, X2_UNTIL, user_settings
    try:
        global_country   = _get_setting("global_country", "RU")
        COOLDOWN_HOURS   = float(_get_setting("cooldown_hours", "3.0"))
        MAINTENANCE_MODE = _get_setting("maintenance_mode", "0") == "1"
        x2_active        = _get_setting("x2_active", "0") == "1"
        x2_until_str     = _get_setting("x2_until", "")
        if x2_active and x2_until_str:
            x2_until = datetime.fromisoformat(x2_until_str)
            if x2_until > datetime.now(timezone.utc):
                X2_ACTIVE = True
                X2_UNTIL  = x2_until
            else:
                X2_ACTIVE = False
        ...
        with get_conn() as conn:
            with conn.cursor() as cur:
                cur.execute("SELECT user_id, country, regions, frame FROM user_settings_db")
                rows = cur.fetchall()
        for r in rows:
            regions = json.loads(r["regions"]) if r["regions"] else None
            user_settings[r["user_id"]] = {
                "country": r["country"],
                "regions": regions,
                "frame":   r["frame"] or "none",
            }
        logger.info(f"[settings] Загружено: country={global_country}, cd={COOLDOWN_HOURS}ч, "
                    f"maintenance={MAINTENANCE_MODE}, x2={X2_ACTIVE}, users={len(user_settings)}")
    except Exception as e:
        logger.error(f"[load_bot_settings] {e}")

def save_global_setting(key: str, value):
    try:
        _set_setting(key, str(value))
    except Exception as e:
        logger.error(f"[save_global_setting] {e}")

def save_user_setting(user_id: int):
    """Сохраняет настройки конкретного пользователя в БД."""
    try:
        s = user_settings.get(user_id, {})
        country = s.get("country", "RU")
        regions = json.dumps(s.get("regions")) if s.get("regions") is not None else None
        frame   = s.get("frame", "none")
        with get_conn() as conn:
            with conn.cursor() as cur:
                cur.execute("""
                    INSERT INTO user_settings_db (user_id, country, regions, frame)
                    VALUES (%s, %s, %s, %s)
                    ON CONFLICT (user_id) DO UPDATE
                    SET country = EXCLUDED.country,
                        regions = EXCLUDED.regions,
                        frame   = EXCLUDED.frame
                """, (user_id, country, regions, frame))
            conn.commit()
    except Exception as e:
        logger.error(f"[save_user_setting] {e}")
...
# Настройки пользователей: {user_id: {"country": ..., "regions": [...] or None}}
user_settings: dict = {}

# Названия регионов (код -> название) — взяты из списка поддерживаемых регионов
RU_REGION_NAMES = {
...
UA_REGION_NAMES = {
    ...
    "CE":"Черновицкая обл.",
    "BA":"Кировоградская обл.","НА":"Кировоградская обл.",
...
CHANNEL_USERNAME = "carzdrop"  # канал для проверки подписки
MSK = timezone(timedelta(hours=3))  # Московское время

# ID которые не участвуют в топе и не получают ответов (анонимные боты, боты каналов)
EXCLUDED_IDS = {1087968824, 136817688}  # GroupAnonymousBot, Channel_Bot
...
(
    ...
    ADMIN_X2_HOURS, ADMIN_BC_SCHEDULE,
) = range(10)
...
def has_purchase(user_id: int, item: str) -> bool:
    try:
        with get_conn() as conn:
            with conn.cursor() as cur:
                cur.execute("SELECT 1 FROM purchases WHERE user_id = %s AND item = %s", (user_id, item))
                return cur.fetchone() is not None
    except Exception:
        _ensure_purchases_table()
        return False

def add_purchase(user_id: int, item: str):
    try:
        with get_conn() as conn:
            with conn.cursor() as cur:
                cur.execute("""
                    INSERT INTO purchases (user_id, item) VALUES (%s, %s)
...
def calc_coins(chars: str, country: str) -> tuple[int, str]:
    """
    Начисляет монеты за номер. Бонусы суммируются:
    - базовые 5⚡ всегда
    - +5⚡ если цифры по порядку (123 или 321)
    - +10⚡ если зеркальные цифры (121)
    - +20⚡ если три одинаковые цифры подряд (111)
    - +20⚡ если четыре одинаковые цифры подряд (1111, включает triple)
    - +20⚡ если три одинаковые буквы подряд (ААА)
    Итого минимум 5, максимум — сумма всех подходящих бонусов.
    """
    base    = 5
    bonus   = 0
    labels  = []

    # Извлекаем только цифры и только буквы из строки номера
    digit_str = "".join(c for c in chars if c.isdigit())
    # Буквы — всё что не цифра и не пробел
    letter_str = "".join(c for c in chars.upper() if c.isalpha())

    # --- Цифровые бонусы ---
    if len(digit_str) >= 3:
        # Четыре одинаковых подряд
        has_quad = any(
            digit_str[i] == digit_str[i+1] == digit_str[i+2] == digit_str[i+3]
            for i in range(len(digit_str) - 3)
        ) if len(digit_str) >= 4 else False
        ...
        # Зеркальные
        is_mirror = (digit_str == digit_str[::-1])

        # По порядку (возрастающие или убывающие)
        digits_list = [int(d) for d in digit_str]
        is_asc  = all(digits_list[i+1] == digits_list[i] + 1 for i in range(len(digits_list)-1))
        is_desc = all(digits_list[i+1] == digits_list[i] - 1 for i in range(len(digits_list)-1))
        is_seq  = is_asc or is_desc

        if has_quad:
            bonus += 20; labels.append("🔥 Четыре подряд")
        if has_triple:
            bonus += 20; labels.append("✨ Тройник")
        if is_mirror and not has_triple:
            bonus += 10; labels.append("🪞 Зеркальный")
        if is_seq and not has_triple and not is_mirror:
            bonus += 5;  labels.append("⭐ По порядку")

    # --- Буквенные бонусы ---
    if len(letter_str) >= 3:
        has_letter_triple = any(
            letter_str[i] == letter_str[i+1] == letter_str[i+2]
            for i in range(len(letter_str) - 2)
        )
        if has_letter_triple:
            bonus += 20; labels.append("🔤 Три буквы")

    total  = base + bonus
    rarity = " + ".join(labels) if labels else "Обычный"
    return total, rarity
...
• Продавай свои номера игрокам на маркете
...
⚡ Система наград:
• Стандартный номер без особенностей — 5⚡
• Цифры идут по порядку (123) — 10⚡
• Зеркальные цифры (121) — 15⚡
• Три одинаковые цифры подряд (111) — 25⚡
• Четыре одинаковые цифры подряд (1111) — 25⚡
• Одинаковые буквы (ааа) — 25⚡

🔗 Полезные ссылки:
• <a href="https://t.me/wovper">наш чат</a>
...
RU_REGIONS = list(RU_REGION_NAMES.keys())
RU_LETTERS = "АВЕКМНОРСТУХ"
UA_REGIONS = list(UA_REGION_NAMES.keys())
UA_LETTERS = "АВЕIКМНОРСТХ"
BY_REGIONS = list(BY_REGION_NAMES.keys())
BY_LETTERS = "АВЕIКМНОРСТХ"
KZ_REGIONS = list(KZ_REGION_NAMES.keys())
...
def generate_plate_image_carbon(country: str, chars: str, region: str) -> bytes:
    """
    Карбоновая рамка: тот же номер что обычный, но:
    - поверх белого поля — диагональные полупрозрачные светло-серые полосы
    - снизу внутри номера — красная плашка с надписью carzplate
    - рамка та же серая как у обычного номера
    """
    # Шаг 1: рисуем стандартный номер
    img, draw, W, H = _base_image()
    cx, cy = W//2, H//2
    pw, ph = 490, 118
    px, py = cx - pw//2, cy - ph//2

    # Размеры красной плашки снизу
    bar_h = 32
    ...
    if country == "RU":
        ...
        # Диагональные полосы поверх белого поля (только левая часть номера)
        stripe_layer = Image.new("RGBA", (W, H), (0, 0, 0, 0))
        sd = ImageDraw.Draw(stripe_layer)
        stripe_w = 28
        gap_w = 28
        alpha = 38  # полупрозрачность
        left_area_w = pw - right_w
        # Полосы идут по всей высоте номера, наклонены ~45°
        for xi in range(px - ph, px + left_area_w + ph, stripe_w + gap_w):
            pts = [
                (xi,            py),
                (xi + stripe_w, py),
                (xi + stripe_w + ph, py + ph),
                (xi + ph,       py + ph),
            ]
            sd.polygon(pts, fill=(255, 255, 255, alpha))
        # Накладываем полосы поверх основного изображения, обрезая по границе номера
        plate_mask = Image.new("L", (W, H), 0)
        pm = ImageDraw.Draw(plate_mask)
        pm.rounded_rectangle([px+4, py+4, px+pw-right_w-2, py+ph-4], radius=6, fill=255)
        img_rgba = img.convert("RGBA")
        img_rgba.paste(stripe_layer, (0, 0), plate_mask)
        img = img_rgba.convert("RGB")
        draw = ImageDraw.Draw(img)

        # Перерисовываем контур номера поверх полос
        draw.rounded_rectangle([px, py, px+pw, py+ph], radius=10, outline="#111111", width=4)
        draw.line([(rdx, py+6),(rdx, py+ph-6)], fill="#111111", width=3)

        # Текст номера
        fnt_pl = _font(FONT_BOLD, 72)
        # Текст рисуем чуть выше центра, т.к. снизу будет плашка
        text_cy = py + (ph - bar_h) // 2
        draw.text((px+(pw-right_w)//2, text_cy), chars, fill="#111111", font=fnt_pl, anchor="mm")

        # Правая колонка (регион, RUS, флаг) — без изменений
        rcx = rdx+right_w//2; rpt = py+6; rpb = py+ph-6; rph_r = rpb-rpt
        fnt_r = _font(FONT_BOLD, 48)
        draw.text((rcx, rpt+int(rph_r*0.42)), region, fill="#111111", font=fnt_r, anchor="mm")
        fw2, fh2 = 22, 15; fnt_rus = _font(FONT_BOLD, 13)
        rus_w = int(fnt_rus.getlength("RUS")); gap = 3; total_w2 = rus_w+gap+fw2
        rus_cy2 = rpt+int(rph_r*0.82); tx = rcx-total_w2//2; fx2 = tx+rus_w+gap; fy2 = rus_cy2-fh2//2
        draw.text((tx, rus_cy2), "RUS", fill="#111111", font=fnt_rus, anchor="lm")
        _ru_flag(draw, fx2, fy2, fw=fw2, fh=fh2)

        # Красная плашка carzplate снизу (только в левой части)
        bar_y = py + ph - bar_h
        draw.rounded_rectangle([px+2, bar_y, px+pw-right_w-2, py+ph-2], radius=8, fill="#CC0000")
        fnt_brand = _font(FONT_BOLD, 17)
        draw.text(((px+2 + px+pw-right_w-2)//2, bar_y + bar_h//2), "carzplate",
                  fill="white", font=fnt_brand, anchor="mm")
        ...
        stripe_layer = Image.new("RGBA", (W, H), (0, 0, 0, 0))
        sd = ImageDraw.Draw(stripe_layer)
        stripe_w = 28; alpha = 38
        for xi in range(px+strip_w - ph, px+pw + ph, stripe_w * 2):
            pts = [(xi, py),(xi+stripe_w, py),(xi+stripe_w+ph, py+ph),(xi+ph, py+ph)]
            sd.polygon(pts, fill=(255, 255, 255, alpha))
        plate_mask = Image.new("L", (W, H), 0)
        pm = ImageDraw.Draw(plate_mask)
        pm.rectangle([px+strip_w+2, py+4, px+pw-4, py+ph-bar_h-2], fill=255)
        img_rgba = img.convert("RGBA")
        img_rgba.paste(stripe_layer, (0, 0), plate_mask)
        img = img_rgba.convert("RGB")
        draw = ImageDraw.Draw(img)
        draw.rounded_rectangle([px, py, px+pw, py+ph], radius=8, outline="#111111", width=4)
        draw.line([(px+strip_w, py+4),(px+strip_w, py+ph-4)], fill="#111111", width=3)
        ...
        fnt_pl = _font(FONT_BOLD, 66)
        text_cy = py + (ph - bar_h) // 2
        draw.text((px+strip_w+(pw-strip_w)//2, text_cy), body, fill="#111111", font=fnt_pl, anchor="mm")

        bar_y = py + ph - bar_h
        draw.rectangle([px+strip_w+2, bar_y, px+pw-2, py+ph-2], fill="#CC0000")
        fnt_brand = _font(FONT_BOLD, 17)
        draw.text(((px+strip_w+2 + px+pw-2)//2, bar_y + bar_h//2), "carzplate",
                  fill="white", font=fnt_brand, anchor="mm")
        ...
        zone_w = 90; fl_w = 72; fl_h = 48
        fnt_by2 = _font(FONT_BOLD, 16); by_bbox = fnt_by2.getbbox("BY")
        by_h = by_bbox[3]-by_bbox[1]; total_h_b = fl_h+4+by_h
        fl_x = px+(zone_w-fl_w)//2; fl_y = py+(ph-total_h_b)//2; red_h = round(fl_h*2/3)
        draw.rectangle([fl_x, fl_y, fl_x+fl_w, fl_y+red_h], fill="#CF101A")
        draw.rectangle([fl_x, fl_y+red_h, fl_x+fl_w, fl_y+fl_h], fill="#007828")
        draw.text((fl_x+fl_w//2, fl_y+fl_h+4), "BY", fill="#111111", font=fnt_by2, anchor="mt")
        ...
        sd = ImageDraw.Draw(stripe_layer)
        stripe_w = 28; alpha = 38
        for xi in range(px+zone_w - ph, px+pw + ph, stripe_w * 2):
            pts = [(xi, py),(xi+stripe_w, py),(xi+stripe_w+ph, py+ph),(xi+ph, py+ph)]
            sd.polygon(pts, fill=(255, 255, 255, alpha))
        plate_mask = Image.new("L", (W, H), 0)
        pm = ImageDraw.Draw(plate_mask)
        pm.rectangle([px+zone_w+2, py+4, px+pw-4, py+ph-bar_h-2], fill=255)
        img_rgba = img.convert("RGBA")
        img_rgba.paste(stripe_layer, (0, 0), plate_mask)
        img = img_rgba.convert("RGB")
        draw = ImageDraw.Draw(img)
        draw.rounded_rectangle([px, py, px+pw, py+ph], radius=8, outline="#111111", width=5)

        c = chars.replace(" ", "").upper()
        body = f"{c[:4]} {c[4:6]}-{region}" if len(c) >= 6 else f"{chars}-{region}"
        fnt_pl = _font(FONT_BOLD, 68)
        text_cy = py + (ph - bar_h) // 2
        draw.text((px+zone_w+6+(px+pw-8-(px+zone_w+6))//2, text_cy), body, fill="#111111", font=fnt_pl, anchor="mm")

        bar_y = py + ph - bar_h
        draw.rectangle([px+zone_w+2, bar_y, px+pw-2, py+ph-2], fill="#CC0000")
        fnt_brand = _font(FONT_BOLD, 17)
        draw.text(((px+zone_w+2 + px+pw-2)//2, bar_y + bar_h//2), "carzplate",
                  fill="white", font=fnt_brand, anchor="mm")
        ...
        draw.rounded_rectangle([px, py, px+pw, py+ph], radius=10, fill="white", outline="#1a1a1a", width=5)
        SW = 90; RW = 64; col_cx = px+SW//2
        fnt_kz_label = _font(FONT_BOLD, 17); flag_drawn = False; flag_bottom = cy
        if KZ_FLAG_PATH and os.path.exists(KZ_FLAG_PATH):
            try:
                flag_img = Image.open(KZ_FLAG_PATH).convert("RGBA")
                target_flag_h = 52; max_flag_w = SW-16
                ...
                total_h_kz = resized.height+3+kz_h
                flag_y = py+(ph-total_h_kz)//2; flag_x = col_cx-resized.width//2
                img.paste(resized, (flag_x, flag_y), resized)
                flag_bottom = flag_y+resized.height; flag_drawn = True
            except Exception:
                pass
        if not flag_drawn:
            fl_w2, fl_h2 = 56, 38
            kz_bbox = fnt_kz_label.getbbox("KZ"); kz_h = kz_bbox[3]-kz_bbox[1]
            total_h_kz = fl_h2+3+kz_h
            flag_y = py+(ph-total_h_kz)//2; fl_x2 = col_cx-fl_w2//2
        ...
        sd = ImageDraw.Draw(stripe_layer)
        stripe_w = 28; alpha = 38
        for xi in range(px+SW - ph, px+rdx + ph, stripe_w * 2):
            pts = [(xi, py),(xi+stripe_w, py),(xi+stripe_w+ph, py+ph),(xi+ph, py+ph)]
            sd.polygon(pts, fill=(255, 255, 255, alpha))
        plate_mask = Image.new("L", (W, H), 0)
        pm = ImageDraw.Draw(plate_mask)
        pm.rectangle([px+SW+2, py+4, rdx-2, py+ph-bar_h-2], fill=255)
        img_rgba = img.convert("RGBA")
        img_rgba.paste(stripe_layer, (0, 0), plate_mask)
        img = img_rgba.convert("RGB")
        draw = ImageDraw.Draw(img)
        draw.rounded_rectangle([px, py, px+pw, py+ph], radius=10, outline="#1a1a1a", width=5)
        draw.line([(rdx, py+10),(rdx, py+ph-10)], fill="#bbbbbb", width=2)
        ...
        fnt_pl = _font(FONT_BOLD, 72)
        text_cy = py + (ph - bar_h) // 2
        draw.text((num_cx, text_cy), body, fill="#111111", font=fnt_pl, anchor="mm")

        bar_y = py + ph - bar_h
        draw.rectangle([px+SW+2, bar_y, rdx-2, py+ph-2], fill="#CC0000")
        fnt_brand = _font(FONT_BOLD, 17)
        draw.text(((px+SW+2 + rdx-2)//2, bar_y + bar_h//2), "carzplate",
                  fill="white", font=fnt_brand, anchor="mm")

    return _finish_image(img, draw, W, H)

def make_random_plate(country: str, forced_regions: list = None, frame: str = "none") -> tuple[bytes, str]:
    if country == "RU":
        if forced_regions:
            chars, _ = _random_ru_plate()
            region = random.choice(forced_regions)
        else:
            chars, region = _random_ru_plate()
    elif country == "UA":
        if forced_regions:
            chars, _ = _random_ua_plate()
            region = random.choice(forced_regions)
        else:
            chars, region = _random_ua_plate()
    elif country == "BY":
        if forced_regions:
            chars, _ = _random_by_plate()
            region = random.choice(forced_regions)
        else:
            chars, region = _random_by_plate()
    elif country == "KZ":
        if forced_regions:
            chars, _ = _random_kz_plate()
            region = random.choice(forced_regions)
        else:
        ...
        chars, region = _random_ru_plate()
    if frame == "carbon":
        return generate_plate_image_carbon(country, chars, region), chars
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [
        [InlineKeyboardButton("🎮 Играть", web_app=WebAppInfo(url="https://snusfnf-png.github.io/cardrop/"), api_kwargs={"style": "primary"})],
        [InlineKeyboardButton("Наш чат", url="https://t.me/wovper", api_kwargs={"style": "danger"}),
         InlineKeyboardButton("Наш канал", url="https://t.me/carzdrop", api_kwargs={"style": "danger"})],
        [InlineKeyboardButton("💬 Бот в чате", callback_data="add_to_chat", api_kwargs={"style": "success"})],
    ]
...
async def cmd_top(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_group:
        # Топ конкретно в этом чате: берём участников чата из users
        # Получаем список user_id через попытку get_chat_member для всех users
        # Проще: топ глобальный, но с пометкой чата — однако у нас нет таблицы связи user↔chat.
        # Реализуем: получаем топ-25 глобально и фильтруем тех, кто есть в чате
        with get_conn() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    "SELECT user_id, username, coins FROM users ORDER BY coins DESC LIMIT 200"
                )
                rows = cur.fetchall()

        lines = [f"🏆 <b>Топ чата по ⚡</b>\n"]
        rank = 0
        shown = 0
        for r in rows:
            uid = r["user_id"]
            try:
                ...
                continue
            if uid in EXCLUDED_IDS or member.user.is_bot:
                continue
            shown += 1
            u = member.user
            uname = u.full_name or u.first_name or f"id{uid}"
            prefix = medals.get(shown, f"{shown}.")
            lines.append(f"{prefix} {uname} — {r['coins']} ⚡")
            if shown >= 25:
                break
        ...
        # Глобальный топ — ЛС или любой другой контекст
        with get_conn() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    "SELECT user_id, username, coins FROM users ORDER BY coins DESC LIMIT 25"
                )
                rows = cur.fetchall()

        lines = [f"🏆 <b>Глобальный топ по ⚡</b>\n"]
        for i, r in enumerate(rows, 1):
            ...
            try:
                ...
                if getattr(chat_info, "is_bot", False) or chat_info.type != "private":
                    continue
                uname = chat_info.full_name or chat_info.first_name or f"id{uid}"
            except Exception:
                uname = f"id{uid}"
...
async def cmd_settings(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.effective_chat.type != "private":
        ...
        return
    await _show_settings_menu(update.message.reply_text)

async def _show_settings_menu(send_fn):
    keyboard = [
        [InlineKeyboardButton("🌍 Страна", callback_data="settings_country")],
        [InlineKeyboardButton("📍 Регион", callback_data="settings_region")],
        [InlineKeyboardButton("🖼 Рамка",  callback_data="settings_frame")],
    ]
    await send_fn("⚙️ <b>Настройки</b>\n\nВыберите раздел:", parse_mode="HTML",
...
async def settings_country_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query; await query.answer()
    uid  = query.from_user.id
    current = user_settings.get(uid, {}).get("country", global_country)
    flags = {"RU": "🇷🇺", "UA": "🇺🇦", "BY": "🇧🇾", "KZ": "🇰🇿"}
    ...
    await query.edit_message_text("🌍 Выбери страну номерного знака:", parse_mode="HTML",
                                   reply_markup=InlineKeyboardMarkup(keyboard))
...
async def settings_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if uid not in user_settings:
        user_settings[uid] = {}
    user_settings[uid]["country"] = code
    # сбрасываем регион при смене страны
    user_settings[uid].pop("regions", None)
    global_country = code  # сохраняем как глобальный дефолт тоже
    save_global_setting("global_country", code)
    save_user_setting(uid)
    flags = {"RU": "🇷🇺", "UA": "🇺🇦", "BY": "🇧🇾", "KZ": "🇰🇿"}
    names = {"RU": "Россия", "UA": "Украина", "BY": "Беларусь", "KZ": "Казахстан"}
    keyboard = []; row = []
...
def _region_keyboard(country: str, selected: set, page: int) -> tuple[list, int, int]:
    """Возвращает (keyboard_rows, current_page, total_pages) — страницы по 25 регионов (5x5)."""
    region_map = {
        "RU": RU_REGION_NAMES, "UA": UA_REGION_NAMES,
        "BY": BY_REGION_NAMES, "KZ": KZ_REGION_NAMES,
    }.get(country, RU_REGION_NAMES)
    codes       = list(region_map.keys())
    page_size   = 25
    total_pages = max(1, (len(codes) + page_size - 1) // page_size)
    page        = max(0, min(page, total_pages - 1))
    slice_      = codes[page * page_size: (page + 1) * page_size]

    keyboard = []
    # Кнопка "Случайный"
    rand_mark = " ✅" if not selected else ""
    keyboard.append([InlineKeyboardButton(f"🎲 Случайный{rand_mark}", callback_data="set_region_random")])
    # Кнопки регионов 5 в ряд
    row = []
    for code in slice_:
        mark = "✓" if code in selected else ""
        label = f"{code}{mark}" if not mark else f"[{code}]"
        row.append(InlineKeyboardButton(label, callback_data=f"toggle_region_{code}"))
        if len(row) == 5:
            keyboard.append(row); row = []
    if row:
        keyboard.append(row)
    ...
    nav = []
    if page > 0:
        nav.append(InlineKeyboardButton("◀", callback_data=f"region_page_{page-1}"))
...
async def settings_region_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query; await query.answer()
    uid     = query.from_user.id
    country = user_settings.get(uid, {}).get("country", global_country)
    selected = set(user_settings.get(uid, {}).get("regions") or [])
    page    = 0
    context.user_data["region_page"] = page

    keyboard, page, total_pages = _region_keyboard(country, selected, page)
    flags    = {"RU": "🇷🇺", "UA": "🇺🇦", "BY": "🇧🇾", "KZ": "🇰🇿"}
    sel_text = "случайный" if not selected else f"выбрано: {', '.join(sorted(selected))}"
    await query.edit_message_text(
        f"📍 <b>Регион</b> {flags.get(country,'')} {country}\n"
        f"Сейчас: {sel_text}\n\n"
        "Выбери один или несколько (или оставь случайный):",
        parse_mode="HTML",
        reply_markup=InlineKeyboardMarkup(keyboard)
    )
...
async def region_page_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query; await query.answer()
    uid     = query.from_user.id
    page    = int(query.data.replace("region_page_", ""))
    context.user_data["region_page"] = page
    country  = user_settings.get(uid, {}).get("country", global_country)
    selected = set(user_settings.get(uid, {}).get("regions") or [])
    keyboard, _, _ = _region_keyboard(country, selected, page)
    flags    = {"RU": "🇷🇺", "UA": "🇺🇦", "BY": "🇧🇾", "KZ": "🇰🇿"}
    sel_text = "случайный" if not selected else f"выбрано: {', '.join(sorted(selected))}"
    await query.edit_message_text(
        f"📍 <b>Регион</b> {flags.get(country,'')} {country}\n"
...
async def toggle_region_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query; await query.answer()
    uid     = query.from_user.id
    code    = query.data.replace("toggle_region_", "")
    if uid not in user_settings:
        user_settings[uid] = {}
    selected = set(user_settings[uid].get("regions") or [])
    if code in selected:
        selected.discard(code)
    else:
        selected.add(code)
    user_settings[uid]["regions"] = list(selected) if selected else None
    save_user_setting(uid)
...
async def set_region_random_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query; await query.answer()
    uid = query.from_user.id
    if uid not in user_settings:
        user_settings[uid] = {}
    user_settings[uid]["regions"] = None
    save_user_setting(uid)
    country  = user_settings.get(uid, {}).get("country", global_country)
    page     = context.user_data.get("region_page", 0)
    keyboard, _, _ = _region_keyboard(country, set(), page)
    flags    = {"RU": "🇷🇺", "UA": "🇺🇦", "BY": "🇧🇾", "KZ": "🇰🇿"}
    await query.edit_message_text(
        f"📍 <b>Регион</b> {flags.get(country,'')} {country}\n"
        f"Сейчас: случайный\n\n"
...
async def settings_frame_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query; await query.answer()
    uid = query.from_user.id
    frame = user_settings.get(uid, {}).get("frame", "none")
    frame_label = "Без улучшений" if frame == "none" else ("Карбоновая рамка" if frame == "carbon" else frame)
    owned_carbon = has_purchase(uid, "carbon")
    keyboard = [
        [InlineKeyboardButton("✅ Без улучшений" if frame == "none" else "Без улучшений", callback_data="frame_none")],
    ]
    if owned_carbon:
        keyboard.append([InlineKeyboardButton(
            "✅ Карбоновая рамка" if frame == "carbon" else "Карбоновая рамка",
            callback_data="frame_set_carbon"
        )])
    keyboard.append([InlineKeyboardButton("◀️ Назад", callback_data="settings_menu")])
    await query.edit_message_text(
        f"🖼 <b>Рамка номерного знака</b>\n\nАктивная рамка: <b>{frame_label}</b>",
        parse_mode="HTML",
        reply_markup=InlineKeyboardMarkup(keyboard)
    )

async def frame_none_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query; await query.answer()
    uid = query.from_user.id
    if uid not in user_settings:
        user_settings[uid] = {}
    user_settings[uid]["frame"] = "none"
    save_user_setting(uid)
    await settings_frame_callback(update, context)

# ── /shop ─────────────────────────────────────────────────────────────────────

SHOP_TEXT = """🛒 <b>Магазин</b>

Здесь ты можешь купить предметы за ⚡"""

async def shop_frames_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query; await query.answer()
    uid = query.from_user.id
    owned = has_purchase(uid, "carbon")
    active_frame = user_settings.get(uid, {}).get("frame", "none")

    carbon_label = "Карбоновая рамка — 400⚡"
    if owned:
        carbon_label = ("✅ " if active_frame == "carbon" else "") + "Карбоновая рамка (куплена)"

    keyboard = [
        [InlineKeyboardButton(carbon_label, callback_data="shop_buy_carbon")],
        [InlineKeyboardButton("◀️ Назад", callback_data="shop_back")],
    ]
    await query.edit_message_text(
        "🖼 <b>Рамки</b>\n\nВыбери рамку для номерного знака:",
        parse_mode="HTML",
        reply_markup=InlineKeyboardMarkup(keyboard)
    )

async def shop_buy_carbon_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query; await query.answer()
    uid = query.from_user.id

    if has_purchase(uid, "carbon"):
        # Уже куплено — просто применяем/снимаем
        if uid not in user_settings:
            user_settings[uid] = {}
        current = user_settings[uid].get("frame", "none")
        if current == "carbon":
            user_settings[uid]["frame"] = "none"
            save_user_setting(uid)
            await query.answer("Карбоновая рамка снята.", show_alert=True)
        else:
            user_settings[uid]["frame"] = "carbon"
            save_user_setting(uid)
            await query.answer("✅ Карбоновая рамка активирована!", show_alert=True)
        await shop_frames_callback(update, context)
        return
    ...
    if coins < PRICE:
        await query.answer(f"❌ Недостаточно монет! Нужно {PRICE}⚡, у тебя {coins}⚡.", show_alert=True)
        return

    # Списываем монеты и добавляем покупку
    new_coins = coins - PRICE
    save_user(uid, new_coins, data.get("last_spin"), data.get("username"))
    add_purchase(uid, "carbon")
    ...
    await query.answer("✅ Карбоновая рамка куплена и активирована!", show_alert=True)
    await shop_frames_callback(update, context)

async def shop_back_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query; await query.answer()
    keyboard = [
        [InlineKeyboardButton("🖼 Рамки", callback_data="shop_frames")],
    ]
    await query.edit_message_text(SHOP_TEXT, parse_mode="HTML",
                                  reply_markup=InlineKeyboardMarkup(keyboard))

async def frame_set_carbon_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query; await query.answer()
    uid = query.from_user.id
    if uid not in user_settings:
        user_settings[uid] = {}
...
async def add_to_chat_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [[InlineKeyboardButton("➕ Добавить бота", url=f"https://t.me/{bot_username}?startgroup=start", api_kwargs={"style": "success"})]]
...
async def adm_group_stats(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for r in rows:
        ...
        if added_dt:
            ...
            added = added_dt.astimezone(MSK).strftime("%d.%m.%Y %H:%M") + " МСК"
        else:
            added = "—"
...
async def adm_maintenance(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    save_global_setting("maintenance_mode", "1" if MAINTENANCE_MODE else "0")
...
async def adm_set_cd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    save_global_setting("cooldown_hours", COOLDOWN_HOURS)
    await query.edit_message_text(
        f"✅ КД изменён на <b>{_cd_label()}</b>\nПрименяется ко всем новым прокруткам.",
        parse_mode="HTML",
        reply_markup=InlineKeyboardMarkup(_back_admin_btn())
    )
...
async def _bc_show_preview(chat_id, context):
    ...
    await context.bot.send_message(
        chat_id, "❓ <b>Когда отправить?</b>", parse_mode="HTML",
        reply_markup=InlineKeyboardMarkup([
            [InlineKeyboardButton("✅ Сейчас",          callback_data="bc_confirm")],
            [InlineKeyboardButton("🕐 По МСК (время)",  callback_data="bc_schedule_msk")],
...
async def adm_bc_confirm(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query; await query.answer()
    if not is_admin(query.from_user): return ConversationHandler.END
    await _do_broadcast(query.message.chat.id, context)
    return ConversationHandler.END

async def adm_bc_schedule_msk(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query; await query.answer()
    if not is_admin(query.from_user): return ConversationHandler.END
    await query.message.reply_text(
        "🕐 Введите время отправки по МСК в формате <b>ЧЧ:ММ</b>\n"
        "Например: <code>10:00</code> или <code>21:30</code>",
        parse_mode="HTML",
        reply_markup=InlineKeyboardMarkup(_back_admin_btn())
    )
    return ADMIN_BC_SCHEDULE

async def adm_bc_schedule_time(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not is_admin(update.effective_user): return ConversationHandler.END
    text = update.message.text.strip()
    import re
    m = re.match(r"^(\d{1,2}):(\d{2})$", text)
    if not m:
        await update.message.reply_text("❌ Неверный формат. Введите время как ЧЧ:ММ, например <code>10:00</code>", parse_mode="HTML")
        return ADMIN_BC_SCHEDULE
    hour, minute = int(m.group(1)), int(m.group(2))
    if not (0 <= hour <= 23 and 0 <= minute <= 59):
        await update.message.reply_text("❌ Неверное время. Попробуйте снова.")
        return ADMIN_BC_SCHEDULE
    ...
    # Сохраняем данные рассылки для отложенной задачи
    bc_data = dict(context.user_data)

    async def _delayed():
        await asyncio.sleep(delay)
        await _do_broadcast(chat_id, context, bc_data)

    asyncio.create_task(_delayed())
    await update.message.reply_text(
        f"✅ Рассылка запланирована на <b>{send_time.strftime('%d.%m %H:%M')} МСК</b>",
        parse_mode="HTML",
        reply_markup=InlineKeyboardMarkup(_back_admin_btn())
    )
    context.user_data.clear()
...
async def _do_broadcast(chat_id: int, context, data: dict = None):
    if data is None:
        data = context.user_data
    text     = data.get("bc_text", "")
    file_id  = data.get("bc_file_id")
    ftype    = data.get("bc_file_type")
    btn_text = data.get("bc_btn_text")
    btn_url  = data.get("bc_btn_url")
    markup   = InlineKeyboardMarkup([[InlineKeyboardButton(btn_text, url=btn_url)]]) if btn_text and btn_url else None
    users    = get_all_users_for_broadcast()
    status   = await context.bot.send_message(chat_id, "📤 <b>Рассылка начата…</b>", parse_mode="HTML")
    ok = fail = 0
    for uid in users:
        try:
    ...
    if action in ("ban", "unban"):
        ...
        if not rows:
            ...
            for r in rows:
                ...
                ts = ts_dt.astimezone(MSK).strftime("%d.%m %H:%M")
                lines.append(f"• {ts} МСК — {r['action']} {r['detail'] or ''}")
            result = "\n".join(lines)
...
async def adm_x2_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if X2_ACTIVE:
        ...
        save_global_setting("x2_active", "0")
        save_global_setting("x2_until", "")
        await query.edit_message_text(
...
async def adm_x2_hours(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    save_global_setting("x2_active", "1")
    save_global_setting("x2_until", X2_UNTIL.isoformat())
    until_msk = X2_UNTIL.astimezone(MSK)
    await update.message.reply_text(
        f"✅ <b>Х2 монеты включён на {hours}ч</b>\nДо: {until_msk.strftime('%d.%m %H:%M')} МСК",
        parse_mode="HTML", reply_markup=InlineKeyboardMarkup(_back_admin_btn())
    )
    return ConversationHandler.END
...
async def handle_nz(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not user:
        return
    if user.is_bot or user.id in EXCLUDED_IDS:
        return
    user_id = user.id
    ...
    try:
        ...
        logger.info(f"[subscription] user {user_id} status in @{CHANNEL_USERNAME}: {member.status}")
        if member.status in ("left", "kicked", "banned"):
            await msg.reply_text(
                '🙁Вы не подписаны на <a href="https://t.me/carzdrop">наш канал</a>, '
                'подпишитесь на него и напишите нз еще раз',
                parse_mode="HTML",
                disable_web_page_preview=True
            )
            return
    except Exception as e:
        logger.warning(f"[subscription check] {e}")
        # При ошибке проверки — пропускаем, не блокируем пользователя
    ...
    try:
        uid_settings = user_settings.get(user_id, {})
        country = uid_settings.get("country", global_country)
        forced_regions = uid_settings.get("regions", None)
        frame = uid_settings.get("frame", "none")
        img_bytes, chars = make_random_plate(country, forced_regions, frame)
        coins_earned, rarity = calc_coins(chars, country)

        check_x2()
        x2_was_active = X2_ACTIVE
        if X2_ACTIVE:
        ...
        if x2_was_active:
            caption = f"<blockquote>+{base_coins} ⚡(🔥x2) |  Всего: {new_coins} ⚡</blockquote>"
        else:
...
async def handle_private_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user.is_bot or user.id in EXCLUDED_IDS:
        return
    # нз работает в ЛС — передаём в handle_nz
    if msg.text.strip().lower() == "нз":
        await handle_nz(update, context)
        return
    if is_admin(user):
        return
    if MAINTENANCE_MODE:
...
# ══════════════════════════════════════════════════════════════════════════════
#  Admin API — отдельный поток на порту 8081
# ══════════════════════════════════════════════════════════════════════════════
...
class AdminAPIHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        try:
            ...
            if not user or (user.get("username") or "").lower() != ADMIN_USERNAME.lower():
                self._respond(401, {"ok": False, "error": "unauthorized"})
                return
            if self.path == "/admin-api/stats":
                self._respond(200, self._get_stats())
            elif self.path == "/admin-api/users":
                self._respond(200, self._get_users())
            elif self.path == "/admin-api/top":
                self._respond(200, self._get_top())
            else:
    ...
    def _get_stats(self):
        with get_conn() as conn:
            with conn.cursor() as cur:
                ...
                cur.execute("SELECT COUNT(*) AS c FROM users WHERE played_webapp=TRUE"); played = cur.fetchone()["c"]
                cur.execute("SELECT COUNT(*) AS c FROM chats"); chats = cur.fetchone()["c"]
                cur.execute("SELECT COUNT(*) AS c FROM users WHERE blocked=TRUE"); blocked = cur.fetchone()["c"]
                days = []
                for i in range(13, -1, -1):
                    ds = datetime.now(timezone.utc).replace(hour=0,minute=0,second=0,microsecond=0) - timedelta(days=i)
                    de = ds + timedelta(days=1)
                    cur.execute("SELECT COUNT(*) AS c FROM users WHERE joined_at>=%s AND joined_at<%s", (ds,de))
                    nu = cur.fetchone()["c"]
                    cur.execute("SELECT COUNT(*) AS c FROM users WHERE played_webapp=TRUE AND joined_at>=%s AND joined_at<%s", (ds,de))
                    pl = cur.fetchone()["c"]
                    days.append({"date": ds.strftime("%d.%m"), "new_users": nu, "played": pl})
        return {"ok": True, "total": total, "played": played, "chats": chats, "blocked": blocked, "days": days}

    def _get_users(self):
        with get_conn() as conn:
            with conn.cursor() as cur:
                cur.execute("SELECT user_id,username,coins,played_webapp,blocked,joined_at FROM users ORDER BY joined_at DESC LIMIT 500")
                rows = [dict(r) for r in cur.fetchall()]
        for r in rows:
            if r.get("joined_at"): r["joined_at"] = r["joined_at"].isoformat()
    ...
    def _get_top(self):
        with get_conn() as conn:
            with conn.cursor() as cur:
                cur.execute("SELECT user_id,username,coins FROM users ORDER BY coins DESC LIMIT 50")
                rows = [dict(r) for r in cur.fetchall()]
        return {"ok": True, "top": rows}

def start_admin_api():
    """Запускает Admin API в отдельном потоке на порту 8081."""
    try:
        server = HTTPServer(("0.0.0.0", 8081), AdminAPIHandler)
        logger.info("Admin API запущен на порту 8081")
...
async def post_init(application: Application):
    init_db()
    load_bot_settings()
    # Удаляем ботов и анонимных пользователей из таблицы users
    with get_conn() as conn:
        with conn.cursor() as cur:
            cur.execute("DELETE FROM users WHERE user_id = ANY(%s)", ([1087968824, 136817688],))
        conn.commit()
    await application.bot.delete_my_commands()

async def maintenance_gate(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Блокирует любые апдейты от не-админов во время тех. перерыва."""
    if MAINTENANCE_MODE and not is_admin(update.effective_user):
        # Для callback_query нужен answer() чтобы убрать "часики"
...
def main():
    ...
    # ── Тех. перерыв — блокирует всё для не-админов (group=-1, выполняется первым) ─
    ...
    broadcast_conv = ConversationHandler(
        ...
        states={
            ...
            ADMIN_BC_BTN_CHOICE: [
                ...
                CallbackQueryHandler(adm_bc_schedule_msk,   pattern=r"^bc_schedule_msk$"),
            ...
            ADMIN_BC_SCHEDULE: [MessageHandler(filters.TEXT & ~filters.COMMAND, adm_bc_schedule_time)],
        },
    ...
    app.add_handler(CallbackQueryHandler(settings_country_callback,  pattern=r"^settings_country$"))
    app.add_handler(CallbackQueryHandler(settings_region_callback,   pattern=r"^settings_region$"))
    app.add_handler(CallbackQueryHandler(settings_frame_callback,    pattern=r"^settings_frame$"))
    app.add_handler(CallbackQueryHandler(toggle_region_callback,     pattern=r"^toggle_region_"))
    app.add_handler(CallbackQueryHandler(region_page_callback,       pattern=r"^region_page_"))
    app.add_handler(CallbackQueryHandler(set_region_random_callback, pattern=r"^set_region_random$"))
    app.add_handler(CallbackQueryHandler(frame_none_callback,        pattern=r"^frame_none$"))
    app.add_handler(CallbackQueryHandler(frame_set_carbon_callback,  pattern=r"^frame_set_carbon$"))
    app.add_handler(CallbackQueryHandler(shop_frames_callback,       pattern=r"^shop_frames$"))
    app.add_handler(CallbackQueryHandler(shop_buy_carbon_callback,   pattern=r"^shop_buy_carbon$"))
    ...
    # Запускаем Admin API в фоновом потоке
    ...
    if WEBHOOK_URL:
        ...
        def _adm_check(body):
            # Пробуем через подписанный initData
            user = _validate_init_data(body.get("initData", ""))
            if user and (user.get("username") or "").lower() == ADMIN_USERNAME.lower():
                return user
            # Fallback: доверяем initDataUnsafe.user если initData не прошёл
            # (на случай если Telegram не передал подпись в тестовом режиме)
            unsafe = body.get("user", {})
            if unsafe and (unsafe.get("username") or "").lower() == ADMIN_USERNAME.lower():
                return unsafe
            return None

        async def route_webhook(request: Request) -> Response:
            try:
                data   = await request.body()
                update = _Update.de_json(json.loads(data), app.bot)
                await app.update_queue.put(update)
            except Exception as e:
                logger.error(f"[webhook] {e}")
            return Response(status_code=200)

        async def route_options(request: Request) -> Response:
        ...
        async def route_stats(request: Request) -> JSONResponse:
            ...
            if not _adm_check(body):
                return JSONResponse({"ok": False, "error": "unauthorized"}, status_code=401, headers=_cors_headers())
            h = AdminAPIHandler.__new__(AdminAPIHandler)
            return JSONResponse(h._get_stats(), headers=_cors_headers())

        async def route_users(request: Request) -> JSONResponse:
            if request.method == "OPTIONS":
                return Response(status_code=200, headers=_cors_headers())
            body = await request.json()
            if not _adm_check(body):
                return JSONResponse({"ok": False, "error": "unauthorized"}, status_code=401, headers=_cors_headers())
            h = AdminAPIHandler.__new__(AdminAPIHandler)
            return JSONResponse(h._get_users(), headers=_cors_headers())

        async def route_top(request: Request) -> JSONResponse:
            if request.method == "OPTIONS":
                return Response(status_code=200, headers=_cors_headers())
            body = await request.json()
            if not _adm_check(body):
                return JSONResponse({"ok": False, "error": "unauthorized"}, status_code=401, headers=_cors_headers())
            h = AdminAPIHandler.__new__(AdminAPIHandler)
            return JSONResponse(h._get_top(), headers=_cors_headers())

        async def route_health(request: Request) -> Response:
            return Response("ok")

        starlette_app = Starlette(routes=[
            Route("/",                    route_health,   methods=["GET"]),
            Route("/health",              route_health,   methods=["GET"]),
            Route(f"/{BOT_TOKEN}",        route_webhook,  methods=["POST"]),
            Route("/admin-api/stats",     route_stats,    methods=["POST", "OPTIONS"]),
            Route("/admin-api/users",     route_users,    methods=["POST", "OPTIONS"]),
            Route("/admin-api/top",       route_top,      methods=["POST", "OPTIONS"]),
        ])
        ...
        async def run_all():
            ...
            print(f"Бот + Admin API запущены на порту {PORT}")
