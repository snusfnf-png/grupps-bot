    ...
    if os.path.exists(DATA_FILE):
        try:
            ...
            for uid, d in raw.items():
                user_data[int(uid)] = {
                    "coins": d.get("coins", 0),
                    "last_spin": datetime.fromisoformat(d["last_spin"]) if d.get("last_spin") else None
                }
        except Exception as e:
            print(f"[DATA] Ошибка загрузки данных: {e}")
...
def _save_data():
    ...
    try:
        ...
        for uid, d in user_data.items():
            raw[str(uid)] = {
                "coins": d["coins"],
                "last_spin": d["last_spin"].isoformat() if d["last_spin"] else None
            }
...
INFO_TEXT = """Напишите нз — бот пришлёт случайный номер выбранной страны. Прокрутка доступна каждые 3ч
