...
def _find_or_download_fonts():
    ...
    # Несколько зеркал на случай недоступности одного
    bold_urls = [
        ...
        if not os.path.exists(dest):
            for url in urls:
                try:
                    ...
                    print(f"Шрифт скачан: {dest} from {url}")
                    break
                except Exception as e:
                    print(f"Не удалось скачать шрифт {url}: {e}")

    # Проверяем что файлы скачаны и не пустые
...
def _font(path, size: int):
    """Безопасная загрузка шрифта с fallback на встроенный PIL-шрифт."""
    if path:
        try:
...
async def handle_nz(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if remaining is not None:
        await msg.reply_text(
            f'<tg-emoji emoji-id="534590655451">⚡</tg-emoji> Подожди ещё {format_cooldown(remaining)} до следующей крутки.',
            parse_mode="HTML"
        )
        return
    ...
    try:
        ...
        caption = (
            f"+{coins_earned} ⚡  |  Всего: {total_coins} ⚡\n"
            f'<tg-emoji emoji-id="534590655451">⚡</tg-emoji> Следующая крутка через {COOLDOWN_HOURS} ч'
        )
        ...
        await msg.reply_text(f"⚠️ Ошибка при генерации номера: {e}")
