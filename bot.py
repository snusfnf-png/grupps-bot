...
# ══════════════════════════════════════════════════════════════════════════════
#  HTTP /track endpoint — вызывается из мини-аппа
# ══════════════════════════════════════════════════════════════════════════════
...
async def _adm_check_auth(request: web.Request) -> tuple[dict|None, web.Response|None]:
    """Проверяет что запрос от @tntks, возвращает (user, None) или (None, error_response)."""
    CORS = {
    ...
    try:
        ...
        return None, web.Response(status=400, text="bad json", headers=CORS)
    user = _validate_init_data(init_data)
    if not user:
        return None, web.Response(status=401, text="unauthorized", headers=CORS)
    uname = (user.get("username") or "").lower()
    if uname != ADMIN_USERNAME.lower():
        return None, web.Response(status=403, text="forbidden", headers=CORS)
    return user, None
...
async def handle_adm_stats(request: web.Request) -> web.Response:
    if request.method == "OPTIONS":
        return web.Response(status=200, headers=_adm_cors())
    user, err = await _adm_check_auth(request)
    if err: return err
    try:
        with get_conn() as conn:
            with conn.cursor() as cur:
                ...
                cur.execute("SELECT COUNT(*) AS cnt FROM users WHERE blocked = TRUE")
                ...
                for i in range(13, -1, -1):
                    ...
                    cur.execute("SELECT COUNT(*) AS cnt FROM users WHERE played_webapp=TRUE AND joined_at >= %s AND joined_at < %s", (d_start, d_end))
                    pl = cur.fetchone()["cnt"]
                    days.append({"date": d_start.strftime("%d.%m"), "new_users": new_u, "played": pl})
        import json as _json
        return web.Response(
            status=200, headers=_adm_cors(),
            content_type="application/json",
            text=_json.dumps({"ok": True, "total": total, "played": played,
                              "chats": chats, "blocked": blocked, "days": days})
        )
    except Exception as e:
        import json as _json
        return web.Response(status=500, headers=_adm_cors(),
                            content_type="application/json",
                            text=_json.dumps({"ok": False, "error": str(e)}))

async def handle_adm_users(request: web.Request) -> web.Response:
    if request.method == "OPTIONS":
        return web.Response(status=200, headers=_adm_cors())
    user, err = await _adm_check_auth(request)
    if err: return err
    try:
        with get_conn() as conn:
            with conn.cursor() as cur:
                cur.execute("""
                    SELECT user_id, username, coins, played_webapp, blocked, joined_at
                    FROM users ORDER BY joined_at DESC LIMIT 500
                """)
        ...
        return web.Response(
            status=200, headers=_adm_cors(),
            content_type="application/json",
            text=_json.dumps({"ok": True, "users": rows})
        )
    except Exception as e:
        import json as _json
        return web.Response(status=500, headers=_adm_cors(),
...
async def handle_adm_top(request: web.Request) -> web.Response:
    if request.method == "OPTIONS":
        return web.Response(status=200, headers=_adm_cors())
    user, err = await _adm_check_auth(request)
    if err: return err
    try:
        ...
        return web.Response(
            status=200, headers=_adm_cors(),
            content_type="application/json",
            text=_json.dumps({"ok": True, "top": rows})
        )
    except Exception as e:
        import json as _json
        return web.Response(status=500, headers=_adm_cors(),
...
async def handle_track(request: web.Request) -> web.Response:
    """POST /track — фиксирует что пользователь открыл мини-апп."""
    # CORS заголовки — нужны для fetch из браузера
    headers = {
        "Access-Control-Allow-Origin": "*",
        "Access-Control-Allow-Methods": "POST, OPTIONS",
        "Access-Control-Allow-Headers": "Content-Type",
    ...
    user = _validate_init_data(init_data)
    if not user:
        # В режиме разработки (без реального initData) просто пропускаем валидацию
        # если передан user_id напрямую (только для отладки)
        uid = None
    else:
        uid = user.get("id")
    ...
    ensure_user(uid, user.get("username"))
    with get_conn() as conn:
        with conn.cursor() as cur:
            cur.execute(
                "UPDATE users SET played_webapp = TRUE WHERE user_id = %s",
        ...
        conn.commit()
    logger.info(f"[track] user {uid} (@{user.get('username')}) opened miniapp")
...
def main():
    ...
    if WEBHOOK_URL:
        # ── Webhook режим: один порт, PTB + aiohttp на одном сервере ──
        from telegram.ext import Application as _App

        # Строим aiohttp приложение вручную
        ...
        async def tg_webhook(request):
            ...
            update = _Update.de_json(_json.loads(data), app.bot)
            await app.process_update(update)
            return web.Response(status=200)

        http_app.router.add_post(f"/{BOT_TOKEN}", tg_webhook)
        http_app.router.add_route("OPTIONS", "/track",           handle_track)
        http_app.router.add_route("POST",    "/track",           handle_track)
        http_app.router.add_route("OPTIONS", "/admin-api/stats", handle_adm_stats)
        http_app.router.add_route("POST",    "/admin-api/stats", handle_adm_stats)
        http_app.router.add_route("OPTIONS", "/admin-api/users", handle_adm_users)
        http_app.router.add_route("POST",    "/admin-api/users", handle_adm_users)
        http_app.router.add_route("OPTIONS", "/admin-api/top",   handle_adm_top)
        http_app.router.add_route("POST",    "/admin-api/top",   handle_adm_top)
        ...
        async def run_all():
            ...
            await site.start()
            print(f"Бот + API запущены на порту {PORT}")
        ...
        # ── Polling режим (локально): отдельный aiohttp на 8081 ──
        http_app = web.Application()
        http_app.router.add_route("OPTIONS", "/track",           handle_track)
        http_app.router.add_route("POST",    "/track",           handle_track)
        http_app.router.add_route("OPTIONS", "/admin-api/stats", handle_adm_stats)
        ...
        async def start_http():
            ...
            await site.start()
            print("API слушает на порту 8081")

        loop = asyncio.get_event_loop()
        loop.run_until_complete(start_http())
