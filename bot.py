...
def generate_plate_image_carbon(country: str, chars: str, region: str) -> bytes:
    """Генерирует номер с карбоновой рамкой (диагональные полосы, надпись carzplate внизу)."""
    img, draw, W, H = _base_image()
    cx, cy = W//2, H//2

    # Карбоновая рамка — чуть больше стандартного номера
    fw, fh = 510, 140
    fx, fy = cx - fw//2, cy - fh//2

    # Тень рамки
    draw.rounded_rectangle([fx+6, fy+6, fx+fw+6, fy+fh+6], radius=14, fill="#111111")

    # Основа рамки — тёмная с карбоновым паттерном
    draw.rounded_rectangle([fx, fy, fx+fw, fy+fh], radius=14, fill="#1a1a1a")

    # Карбоновые диагональные полосы (имитация карбона)
    from PIL import ImageDraw as _ID
    carbon_tile_size = 12
    stripe_color_dark  = "#141414"
    stripe_color_light = "#2a2a2a"

    # Рисуем диагональные полосы по всей рамке (clip снаружи номера)
    for xi in range(fx - fh, fx + fw + fh, carbon_tile_size * 2):
        # Тёмная полоса
        pts = [
            (xi,            fy),
            (xi + carbon_tile_size, fy),
            (xi + carbon_tile_size + fh, fy + fh),
            (xi + fh,       fy + fh),
        ]
        draw.polygon(pts, fill=stripe_color_dark)
        # Светлая полоса
        pts2 = [
            (xi + carbon_tile_size,           fy),
            (xi + carbon_tile_size * 2,       fy),
            (xi + carbon_tile_size * 2 + fh,  fy + fh),
            (xi + carbon_tile_size + fh,      fy + fh),
        ]
        draw.polygon(pts2, fill=stripe_color_light)

    # Перерисовываем закруглённую рамку поверх полос (обрезаем)
    # Сначала рисуем маску закруглённого прямоугольника
    mask_img = Image.new("L", (W, H), 0)
    mask_draw = ImageDraw.Draw(mask_img)
    mask_draw.rounded_rectangle([fx, fy, fx+fw, fy+fh], radius=14, fill=255)
    # Применяем маску — всё за пределами рамки очищаем
    bg_img, bg_draw, _, _ = _base_image()
    img_array = img.copy()
    # Наложим карбон через композитинг
    carbon_img = Image.new("RGB", (W, H), "#1a1a1a")
    carbon_draw = ImageDraw.Draw(carbon_img)
    for xi in range(fx - fh, fx + fw + fh, carbon_tile_size * 2):
        pts = [
            (xi,            fy),
            (xi + carbon_tile_size, fy),
        ...
        carbon_draw.polygon(pts, fill=stripe_color_dark)
        pts2 = [
            (xi + carbon_tile_size,           fy),
            (xi + carbon_tile_size * 2,       fy),
            (xi + carbon_tile_size * 2 + fh,  fy + fh),
        ...
        carbon_draw.polygon(pts2, fill=stripe_color_light)

    # Объединяем: карбоновые полосы только внутри маски
    img.paste(carbon_img, (0, 0), mask_img)
    draw = ImageDraw.Draw(img)

    # Граница рамки
    draw.rounded_rectangle([fx, fy, fx+fw, fy+fh], radius=14, outline="#444444", width=3)

    # Красная нижняя плашка с надписью carzplate
    bar_h = 22
    bar_y = fy + fh - bar_h - 4
    bar_x1, bar_x2 = fx + 4, fx + fw - 4
    draw.rounded_rectangle([bar_x1, bar_y, bar_x2, bar_y + bar_h], radius=6, fill="#CC0000")
    fnt_brand = _font(FONT_BOLD, 14)
    draw.text(((bar_x1 + bar_x2) // 2, bar_y + bar_h // 2), "carzplate", fill="white",
              font=fnt_brand, anchor="mm")

    # Белое поле номера внутри рамки
    pw, ph = 472, 102
    px, py_p = cx - pw//2, cy - ph//2 - 8

    if country == "RU":
        draw.rounded_rectangle([px, py_p, px+pw, py_p+ph], radius=8, fill="white", outline="#111111", width=3)
        right_w = 100; rdx = px+pw-right_w
        draw.line([(rdx, py_p+5),(rdx, py_p+ph-5)], fill="#111111", width=2)
        fnt_pl = _font(FONT_BOLD, 64)
        draw.text((px+(pw-right_w)//2, py_p+ph//2), chars, fill="#111111", font=fnt_pl, anchor="mm")
        rcx = rdx+right_w//2; rpt = py_p+5; rpb = py_p+ph-5; rph = rpb-rpt
        fnt_r = _font(FONT_BOLD, 42)
        draw.text((rcx, rpt+int(rph*0.42)), region, fill="#111111", font=fnt_r, anchor="mm")
        fw2, fh2 = 20, 13; fnt_rus = _font(FONT_BOLD, 12)
        rus_w = int(fnt_rus.getlength("RUS")); gap = 3; total_w2 = rus_w+gap+fw2
        rus_cy2 = rpt+int(rph*0.82); tx = rcx-total_w2//2; fx2 = tx+rus_w+gap; fy2 = rus_cy2-fh2//2
        ...
        draw.rounded_rectangle([px, py_p, px+pw, py_p+ph], radius=8, fill="white", outline="#111111", width=3)
        strip_w = 56
        draw.rounded_rectangle([px+2, py_p+2, px+strip_w, py_p+ph-2], radius=7, fill="#003DA5")
        draw.line([(px+strip_w, py_p+4),(px+strip_w, py_p+ph-4)], fill="#111111", width=2)
        ffw, ffh = 34, 22; ffx = px+(strip_w-ffw)//2; ffy = py_p+14
        draw.rectangle([ffx, ffy, ffx+ffw, ffy+ffh//2], fill="#005BBB")
        draw.rectangle([ffx, ffy+ffh//2, ffx+ffw, ffy+ffh], fill="#FFD500")
        fnt_ua = _font(FONT_BOLD, 14)
        draw.text((px+strip_w//2, py_p+ph-12), "UA", fill="white", font=fnt_ua, anchor="mm")
        c = chars.strip().upper().replace(" ", "")
        body = f"{region} {c[:4]} {c[4:]}" if len(c) >= 6 else f"{region} {c}"
        fnt_pl = _font(FONT_BOLD, 58)
        draw.text((px+strip_w+(pw-strip_w)//2, py_p+ph//2), body, fill="#111111", font=fnt_pl, anchor="mm")

    elif country == "BY":
        draw.rounded_rectangle([px, py_p, px+pw, py_p+ph], radius=8, fill="white", outline="#111111", width=3)
        zone_w = 82; fl_w = 64; fl_h = 42
        fnt_by2 = _font(FONT_BOLD, 14); by_bbox = fnt_by2.getbbox("BY")
        by_h = by_bbox[3]-by_bbox[1]; total_h = fl_h+4+by_h
        fl_x = px+(zone_w-fl_w)//2; fl_y = py_p+(ph-total_h)//2; red_h = round(fl_h*2/3)
        draw.rectangle([fl_x, fl_y, fl_x+fl_w, fl_y+red_h], fill="#CF101A")
        draw.rectangle([fl_x, fl_y+red_h, fl_x+fl_w, fl_y+fl_h], fill="#007828")
        draw.text((fl_x+fl_w//2, fl_y+fl_h+4), "BY", fill="#111111", font=fnt_by2, anchor="mt")
        c = chars.replace(" ", "").upper()
        body = f"{c[:4]} {c[4:6]}-{region}" if len(c) >= 6 else f"{chars}-{region}"
        fnt_pl = _font(FONT_BOLD, 60)
        draw.text((px+zone_w+4+(px+pw-6-(px+zone_w+4))//2, py_p+ph//2), body, fill="#111111", font=fnt_pl, anchor="mm")

    elif country == "KZ":
        draw.rounded_rectangle([px, py_p, px+pw, py_p+ph], radius=8, fill="white", outline="#111111", width=3)
        SW = 82; RW = 56; col_cx = px+SW//2
        fnt_kz_label = _font(FONT_BOLD, 14)
        if KZ_FLAG_PATH and os.path.exists(KZ_FLAG_PATH):
            try:
                flag_img = Image.open(KZ_FLAG_PATH).convert("RGBA")
                target_flag_h = 44; max_flag_w = SW-14
                ratio_h = target_flag_h/flag_img.height
                new_w = int(flag_img.width*ratio_h)
                if new_w > max_flag_w: ratio_h = max_flag_w/flag_img.width
                new_w = max(1, int(flag_img.width*ratio_h)); new_h = max(1, int(flag_img.height*ratio_h))
                resized = flag_img.resize((new_w, new_h), Image.LANCZOS)
                flag_y = py_p+(ph-new_h)//2 - 6; flag_x = col_cx-resized.width//2
                img.paste(resized, (flag_x, flag_y), resized)
                draw = ImageDraw.Draw(img)
                draw.text((col_cx, flag_y+new_h+2), "KZ", fill="#111111", font=fnt_kz_label, anchor="mt")
            except Exception:
                pass
        rdx = px+pw-RW
        draw.line([(rdx, py_p+8),(rdx, py_p+ph-8)], fill="#bbbbbb", width=2)
        fnt_reg = _font(FONT_BOLD, 36)
        draw.text((rdx+RW//2, py_p+ph//2), region, fill="#111111", font=fnt_reg, anchor="mm")
        c = chars.replace(" ", "")
        body = f"{c[:3]} {c[3:]}" if len(c) == 6 else chars
        num_cx = px+SW+(rdx-px-SW)//2
        fnt_pl = _font(FONT_BOLD, 64)
        draw.text((num_cx, py_p+ph//2), body, fill="#111111", font=fnt_pl, anchor="mm")
...
async def frame_none_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    save_user_setting(uid)
    keyboard = [
        [InlineKeyboardButton("✅ Без улучшений", callback_data="frame_none")],
        [InlineKeyboardButton("◀️ Назад", callback_data="settings_menu")],
    ]
    await query.edit_message_text(
        "🖼 <b>Рамка номерного знака</b>\n\nАктивная рамка: <b>Без улучшений</b>",
        parse_mode="HTML",
        reply_markup=InlineKeyboardMarkup(keyboard)
    )
...
Здесь ты можешь купить предметы за ⚡ монеты.
После покупки предмет применяется автоматически.
Изменить выбор можно в /settings → Рамка."""
