...
⚡ <b>Система наград</b>
За каждую прокрутку вы получаете ⚡ в зависимости от редкости номера:

• <b>Обычный</b> — +5 ⚡ (стандартный номер без особенностей)
• <b>⭐ Красивый</b> — +10 ⚡ (цифры идут по порядку, например 1-2-3 или 5-4-3)
• <b>🪞 Зеркальный</b> — +15 ⚡ (цифры читаются одинаково в обе стороны)
• <b>✨ Тройник</b> — +25 ⚡ (три одинаковые цифры подряд, например 777)
• <b>🔥 Четвёрка</b> — +40 ⚡ (четыре одинаковые цифры подряд, например 8888)
• <b>🏆 Легендарный</b> — +100 ⚡ (все три цифры одинаковые, например 000 или 999)
...
async def settings_region_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = []
    # Кнопка "Случайный регион"
    rand_mark = " ✅" if not selected else ""
    keyboard.append([InlineKeyboardButton(f"🎲 Случайный регион{rand_mark}", callback_data="set_region_random")])

    # Кнопки регионов по 3 в ряд
    row = []
    for code in region_map:
        mark = " ✓" if code in selected else ""
        row.append(InlineKeyboardButton(f"{code}{mark}", callback_data=f"toggle_region_{code}"))
        if len(row) == 3:
            keyboard.append(row); row = []
    if row:
        keyboard.append(row)
    ...
    await query.edit_message_text(
        ...
        f"Сейчас: {sel_text}\n\n"
        "Выбери один или несколько регионов (или оставь случайный):",
        parse_mode="HTML",
        reply_markup=InlineKeyboardMarkup(keyboard)
    )
...
async def toggle_region_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Перерисовываем то же меню регионов
    country = user_settings.get(uid, {}).get("country", global_country)
    region_map = {
    ...
    for c in region_map:
        ...
        row.append(InlineKeyboardButton(f"{c}{mark}", callback_data=f"toggle_region_{c}"))
        if len(row) == 3:
            keyboard.append(row); row = []
    if row:
        keyboard.append(row)
...
async def set_region_random_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [[InlineKeyboardButton("🎲 Случайный регион ✅", callback_data="set_region_random")]]
    row = []
    for c in region_map:
        row.append(InlineKeyboardButton(c, callback_data=f"toggle_region_{c}"))
        if len(row) == 3:
            keyboard.append(row); row = []
    if row:
        keyboard.append(row)
...
async def settings_frame_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.edit_message_text(
        ...
        "<i>В будущем здесь появятся рамки из магазина.</i>",
        parse_mode="HTML",
        reply_markup=InlineKeyboardMarkup(keyboard)
    )
