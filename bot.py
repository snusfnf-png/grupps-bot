...
def main():
    ...
    TRACK_PORT  = int(os.environ.get("TRACK_PORT", 8081))

    # Запускаем aiohttp сервер для /track endpoint (отдельный порт)
    ...
    async def run_http():
        ...
        await runner.setup()
        site = web.TCPSite(runner, "0.0.0.0", TRACK_PORT)
        await site.start()
        print(f"HTTP /track слушает на порту {TRACK_PORT}")
